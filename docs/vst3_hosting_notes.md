...
- `PluginEditor` は separate window と embedded の 2 モードをサポート
- embedded 失敗時は自動的に separate window にフォールバック（`src/app/rack_view.rs::ensure_inline_rack_editor()`）
- editor のライフサイクル管理は `src/vst_host/editor/mod.rs`
- SEH 保護・Win32 ウィンドウ管理は `src/vst_host/editor/win32.rs`
- IPlugFrame COM 実装（resize_view）は `src/vst_host/editor/host_frame.rs`
...
この smoke test は実プラグインをロードし、`setup_processing()` のあと editor を開いて短時間維持し、close まで確認する。embedded 版は open→close→reopen の往復テストも含む。
