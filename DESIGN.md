...
- `METRONOME`, `LOOPER`, `BACKING`, `STATUS` の塊に分ける
...
- `src/app/`
  ...
  - `rack_view.rs` — Rack View / Node Editor のメイン描画、VST パラメータパネル
  - `rack.rs` — グラフ↔ラック同期、シグナルチェーン構築
  - `commands.rs` — EdCmd 処理、Undo/Redo
...
- `src/ui/controls.rs`
  ノブ、トグル、今後のボタン部品を扱う
- `src/ui/preferences/midi_tab.rs`
  MIDI設定タブ。デバイス選択、MIDI Learn、バインディング一覧。transport hardware の一部として扱う。
